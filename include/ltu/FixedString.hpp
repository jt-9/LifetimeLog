...
public:
  ...
  // modifiers
  // cppcheck-suppress [functionConst, constParameterReference]
...
// hash support
template<std::size_t N> struct std::hash<ltu::fixed_string<N>> : std::hash<std::string_view>
{
};
template<std::size_t N> struct std::hash<ltu::fixed_u8string<N>> : std::hash<std::u8string_view>
{
};
template<std::size_t N> struct std::hash<ltu::fixed_u16string<N>> : std::hash<std::u16string_view>
{
};
template<std::size_t N> struct std::hash<ltu::fixed_u32string<N>> : std::hash<std::u32string_view>
{
};
...
template<std::size_t N> struct std::hash<ltu::fixed_wstring<N>> : std::hash<std::basic_string_view<wchar_t>>
{
};

// formatting support
// template<typename CharT, std::size_t N>
// struct std::formatter<ltu::basic_fixed_string<CharT, N>> : formatter<std::basic_string_view<CharT>>
//{
//  template<typename FormatContext>
//  auto format(const ltu::basic_fixed_string<CharT, N> &str, FormatContext &ctx) const -> decltype(ctx.out())
//  {
